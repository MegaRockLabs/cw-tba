...
use cw_tba::{verify_nft_ownership, TokenInfo, 
...
use crate::{
    error::ContractError, registry::construct_label, state::{LAST_ATTEMPTING, REGISTRY_PARAMS, TOKEN_ADDRESSES}
};
...
pub fn create_account<T: Serialize>(
    ...
    if !REGISTRY_PARAMS.load(deps.storage)?.allowed_code_ids.contains(&code_id) {
        return Err(ContractError::InvalidCodeId {})
    }
    let is_manager = REGISTRY_PARAMS.load(deps.storage)?.managers.contains(&info.sender.to_string());
...
pub fn update_account_owner(
    ...
    let is_manager = REGISTRY_PARAMS.load(deps.storage)?.managers.contains(&sender.to_string());
...
pub fn migrate_account(
    ...
    if !REGISTRY_PARAMS.load(deps.storage)?.allowed_code_ids.contains(&new_code_id) {
        return Err(ContractError::InvalidCodeId {})
    }
