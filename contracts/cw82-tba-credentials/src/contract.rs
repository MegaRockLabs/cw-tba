...
use crate::{
    ...
    query::{
        assets, can_execute, can_execute_signed, full_info, known_tokens, valid_signature,
       /*  valid_signatures,  */
    },
 ...
 */
    ...
    Ok(
        res, //    .add_attribute("action_name", action_name)
    )
}
...
pub fn query(deps: Deps, env: Env, msg: QueryMsg) -> StdResult<Binary> {
    ...
    match msg {
        ...
        QueryMsg::CanExecuteSigned { msg, signed } => {
            to_json_binary(&can_execute_signed(deps, env, signed, msg)?)
        }
...
#[cfg(feature = "archway")]
#[cfg_attr(not(feature = "library"), entry_point)]
pub fn sudo(deps: DepsMut, env: Env, msg: crate::msg::SudoMsg) -> ContractResult {
    return match msg {
        crate::msg::SudoMsg::CwGrant(grant) => crate::grants::cwfee_grant(deps, env, grant),
    };
}
